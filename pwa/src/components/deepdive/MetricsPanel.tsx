    ...
    <CollapsiblePanel title="Metrics & Valuation" preview={preview}>
      ...
      {p && (
        <div style={{ marginBottom: "var(--space-md)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "var(--space-md)" }}>
            {p.trailingPE != null && <Metric label="P/E (TTM)" value={p.trailingPE.toFixed(1)} mono glossary="p/e ratio" />}
            {p.forwardPE != null && <Metric label="P/E (Fwd)" value={p.forwardPE.toFixed(1)} mono glossary="forward p/e" />}
            {p.priceToBook != null && <Metric label="P/B" value={p.priceToBook.toFixed(2)} mono glossary="price to book" />}
            {p.priceToSales != null && <Metric label="P/S" value={p.priceToSales.toFixed(2)} mono glossary="price to sales" />}
            {f?.earnings_yield != null && <Metric label="Earnings Yield" value={`${(f.earnings_yield * 100).toFixed(1)}%`} mono glossary="earnings yield" />}
            {f?.roic != null && <Metric label="ROIC" value={`${(f.roic * 100).toFixed(0)}%`} mono glossary="roic" />}
            {p.beta != null && <Metric label="Beta" value={p.beta.toFixed(2)} mono glossary="beta" />}
            {p.dividendYield != null && <Metric label="Div Yield" value={`${p.dividendYield.toFixed(2)}%`} mono glossary="dividend yield" />}
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "var(--space-md)", marginBottom: "var(--space-md)" }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: "var(--text-xs)", color: "var(--color-text-muted)", marginBottom: "var(--space-xs)", display: "flex", alignItems: "center", justifyContent: "center", gap: 2 }}>Composite<GlossaryTooltip term="composite score" /></div>
              <div style={{
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: "var(--text-xs)", color: "var(--color-text-muted)", marginBottom: "var(--space-xs)", display: "flex", alignItems: "center", justifyContent: "center", gap: 2 }}>Piotroski F<GlossaryTooltip term="piotroski f-score" /></div>
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: "var(--text-xs)", color: "var(--color-text-muted)", marginBottom: "var(--space-xs)", display: "flex", alignItems: "center", justifyContent: "center", gap: 2 }}>Altman Z<GlossaryTooltip term="altman z-score" /></div>
      ...
      {f && (
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "var(--space-md)" }}>
            <Metric label="Market Cap" value={formatCap(f.market_cap)} mono glossary="market cap" />
            <Metric label="EV" value={formatCap(f.enterprise_value)} mono glossary="enterprise value" />
            <Metric label="Revenue" value={formatNum(f.revenue)} mono glossary="revenue" />
            <Metric label="Net Income" value={formatNum(f.net_income)} mono glossary="net income" />
            <Metric label="Op Income" value={formatNum(f.operating_income)} mono glossary="operating income" />
