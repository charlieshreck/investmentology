...
# Paths
STATE_DIR = Path("/home/investmentology/collaborate/state")
LOGS_DIR = Path("/home/investmentology/collaborate/logs")
...
"""
    ...
    def invoke_gemini(self, action: str, payload: Dict) -> Dict:
        ...
        cmd = [
            ...
            "-o", "json",
            "--yolo",
            "--allowed-mcp-server-names", "knowledge"  # Limit to knowledge-mcp to stay under 512 tool limit
        ]
    ...
    def detect_conflict(self, gemini_response: Dict) -> Optional[Conflict]:
        ...
        for blocker in blockers:
            # Handle both dict blockers (structured) and string blockers (simple)
            if isinstance(blocker, str):
                # String blocker - treat as a simple issue description
                blocker_severity = "major"  # Assume major if just a string
                blocker_issue = blocker
                blocker_position = ""
                blocker_description = blocker
            elif isinstance(blocker, dict):
                blocker_severity = blocker.get("severity", "minor")
                blocker_issue = blocker.get("issue", "Unknown issue")
                blocker_position = blocker.get("my_position", "")
                blocker_description = blocker.get("description", "")
            else:
                # Skip unknown types
                continue

            if blocker_severity in ["blocker", "major"]:
                if self.state.iteration >= self.MAX_ITERATIONS:
                    return Conflict(
                        ...
                        gemini_position=Position(
                            agent="gemini",
                            recommendation=blocker_position,
                            confidence=gemini_response.get("confidence", 0.8),
                            rationale=blocker_description
                        ),
